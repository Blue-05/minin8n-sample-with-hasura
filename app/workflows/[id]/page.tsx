...
export default function WorkflowDetail() {
  ...
  async function load() {
    try {
      ...
      if (!w.workflows_by_pk) {
        setError("Workflow not found or not visible to this user.");
        return;
      }
      ...
      if (latest) {
        setRunId(latest.id);
        const s = await gql<{ step_runs: StepRun[] }>(STEP_RUNS_QUERY, {
          runId: latest.id,
        });
  ...
  const role =
    workflow && user
      ? members.find(
          (m) => m.org_id === workflow.org_id && m.user_id === user.id
        )?.role
      : null;
  const canEdit = role === "owner" || role === "editor";
  const isOwner = role === "owner";

  // Derive deterministically sorted steps array
  const sortedSteps = workflow?.workflow_steps
    ? [...workflow.workflow_steps].sort((a, b) => a.position - b.position)
    : [];
  ...
  async function addStep(type: StepType) {
    ...
    try {
      const positions = workflow.workflow_steps.map((step) => step.position);
  ...
  async function updateStep(s: Step, config: string) {
    try {
      await gql(UPDATE_STEP, {
        id: s.id,
        changes: { config: JSON.parse(config) },
      });
      await load();
    } catch (e: any) {
      setError("Config must be valid JSON: " + e.message);
    }
  }

  async function removeStep(s: Step) {
    try {
      await gql(DELETE_STEP, { id: s.id });
      await load();
    } catch (e: any) {
      setError(e.message);
    }
  ...
  async function moveStep(index: number, dir: -1 | 1) {
    if (!workflow || sortedSteps.length === 0) return;
    ...
    try {
      await gql(UPDATE_STEP, {
        id: a.id,
        changes: { position: -100 - index },
      });
      await gql(UPDATE_STEP, {
        id: b.id,
        changes: { position: a.position },
      });
      await gql(UPDATE_STEP, {
        id: a.id,
        changes: { position: b.position },
      });
      await load();
    } catch (e: any) {
      setError(e.message);
  ...
  async function removeTrigger(t: any) {
    try {
      await gql(DELETE_TRIGGER, { id: t.id });
      await load();
    } catch (e: any) {
      setError(e.message);
    }
  ...
  async function run() {
    ...
    try {
      ...
      }>(TRIGGER_RUN, { workflow_id: id });
      if (!r.triggerWorkflowRun.success)
        throw new Error(r.triggerWorkflowRun.error || "Run failed");
      if (r.triggerWorkflowRun.workflow_run_id)
        setRunId(r.triggerWorkflowRun.workflow_run_id);
      await load();
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  async function approve(sr: StepRun) {
    setBusy(true);
    try {
      const r = await gql<{
        approveStep: {
      ...
      }>(APPROVE_STEP, { step_run_id: sr.id });
      if (!r.approveStep.success)
        throw new Error(r.approveStep.error || "Approval failed");
      await load();
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  if (!workflow)
    return (
      <main className="container">
        <div className="error">{error || "Loading workflow…"}</div>
      </main>
    );
  ...
  return (
    <main className="container">
      ...
      <div
        className="grid"
        style={{ marginTop: 20, gridTemplateColumns: "1.4fr .8fr" }}
      >
              ...
              <div className="row">
                {(
                  ...
                  <button
                    ...
                    key={t}
                    onClick={() => addStep(t)}
                    disabled={(t === "db_write" || t === "notify") && !isOwner}
                  >
                    + {t}
                  </button>
                ))}
            ...
            {sortedSteps.map((s, i) => (
              <div className="step" key={s.id}>
                <div className="stephead">
                  ...
                  <span className="badge">position {s.position}</span>
                  {(s.type === "db_write" || s.type === "notify") && (
                ...
                {canEdit ? (
                  <textarea
                    className="textarea"
                    style={{ marginTop: 10 }}
                    defaultValue={pretty(s.config)}
                    onBlur={(e) => updateStep(s, e.target.value)}
                  />
                ) : (
                  <pre>{pretty(s.config)}</pre>
                )}
              </div>
            ))}
          </div>
        ...
        <aside className="card">
          ...
          {canEdit && (
            <button
              ...
              disabled={!isOwner}
            >
              + webhook trigger {isOwner ? "" : "(owner only)"}
            </button>
          )}
          <hr style={{ borderColor: "var(--line)", margin: "20px 0" }} />
          <h2>Usage</h2>
          <p className="muted">
            Org quota is enforced in the Action function. Use the Nhost
            dashboard to inspect the authoritative values.
          </p>
        </aside>
      </div>
      <RunMonitor
...
function RunMonitor({
  ...
  runId: string | null;
  stepRuns: StepRun[];
  onApprove: (s: StepRun) => void;
  canApprove: boolean;
}) {
  if (!runId) return null;
  const status =
    workflow.workflow_runs.find((r) => r.id === runId)?.status || "live";

  // Map step runs to workflow steps and sort by configured step position
  const stepMap = new Map(workflow.workflow_steps.map((s) => [s.id, s]));
  const sortedStepRuns = [...stepRuns].sort((a, b) => {
    const posA = stepMap.get(a.workflow_step_id)?.position ?? 0;
    const posB = stepMap.get(b.workflow_step_id)?.position ?? 0;
    return posA - posB;
  });
  ...
  return (
    <section className="card" style={{ marginTop: 20 }}>
      ...
      <div className="steps">
        {sortedStepRuns.map((r, i) => {
          const step = stepMap.get(r.workflow_step_id);
          return (
            <div className="step" key={r.id}>
              <div className="row space">
                <div>
                  <strong>
                    {i + 1}. {step?.type || r.workflow_step_id}
                  </strong>
                  <div className={`status-${r.status}`}>
                    {r.status} · attempts {r.attempt_count}
                  </div>
                </div>
                {r.status === "paused" && canApprove && (
                  <button className="btn primary" onClick={() => onApprove(r)}>
                    Approve
                  </button>
                )}
              </div>
